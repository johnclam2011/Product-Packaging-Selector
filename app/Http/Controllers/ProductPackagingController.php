...
class ProductPackagingController extends Controller
{
    ...
    public function optionPackage(Request $request)
    {
        $products = $request->input('products');
        $boxes = $this->boxes;

        $packedBoxes = $this->packProducts($products, $boxes);

        if ($packedBoxes === false) {
            return back()->withErrors('One or more products do not fit in any available box.');
        }

        return view('results', compact('packedBoxes'));
    }

    private function packProducts($products, $boxes)
    {
